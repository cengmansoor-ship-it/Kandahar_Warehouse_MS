  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <button 
          onClick={() => toast.info(t('tender_creation_soon'))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        <ProcurementStatsCard title={t('active_tenders_stats')} value="12" icon={<Gavel size={24} className="text-primary-teal" />} />
        <ProcurementStatsCard title={t('quotation_reviews_stats')} value="48" icon={<FileCheck2 size={24} className="text-amber-500" />} />
        <ProcurementStatsCard title={t('new_vendors_stats')} value="3" icon={<UserCheck size={24} className="text-blue-500" />} />
      </div>
      ...
      <div className="fintech-card bg-white mt-8 overflow-hidden">
        <div className="p-6 lg:p-8 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
             ...
             <h3 className="font-black text-xl text-slate-900 tracking-tight">{t('active_tenders_bids')}</h3>
          </div>
          <div className="relative w-full md:w-72 text-start">
            <Search className={cn("absolute top-1/2 -translate-y-1/2 text-slate-300", t('lang_direction') === 'rtl' ? "right-4" : "left-4")} size={16} />
            <input 
              type="text" 
              placeholder={t('filter_tender_placeholder')}
              className={cn(
                "w-full bg-slate-50 border border-slate-100 rounded-2xl py-3.5 text-[10px] font-black uppercase tracking-widest outline-none focus:ring-4 focus:ring-primary-teal/5 transition-all text-slate-700",
          ...
          <table className="w-full text-start border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap text-start">{t('tender_info')}</th>
                <th className="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap text-start">{t('quotations')}</th>
                <th className="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap text-start">{t('est_budget')}</th>
                <th className="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap text-start">{t('deadline')}</th>
                <th className="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap text-end">{t('status')}</th>
              </tr>
            </thead>
                ...
                <tr key={tender.id} className="hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="px-8 py-5 text-start">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-400 italic">{t('submitted')}</span>
                    </div>
                  </td>
                  <td className="px-8 py-5 text-start">
                  ...
                  <td className="px-8 py-6 text-end">
                    <div className={cn("flex items-center gap-5 justify-end")}>
                      <span className={cn(
                        ...
                        {tender.status === 'Awarded' ? t('status_awarded') : 
                         tender.status === 'Review' ? t('status_review') : 
                         t('status_bidding')}
                      </span>
                      <button 
                        onClick={() => toast.success(`${t('opening_tender_details')} ${tender.id}`)}
